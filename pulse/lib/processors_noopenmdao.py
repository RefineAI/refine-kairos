...
class findFaceGetPulse(object):
    ...
    def draw_rect(self, rect, col=(238, 206, 84)):
        x, y, w, h = rect
    ...
    def run(self, cam):
        ...
        self.gray = cv2.equalizeHist(cv2.cvtColor(self.frame_in,
                                                  cv2.COLOR_BGR2GRAY))
        col = (238, 206, 84)
        #print("Find Faces is %s" % self.find_faces)
        if self.find_faces:
            cv2.putText(
            ...
            #print("length of detected array %s" % len(detected))
            if len(detected) > 0:
            ...
            self.draw_rect(self.face_rect, col=(238, 206, 84))
            x, y, w, h = self.face_rect
            ...
            cv2.putText(self.frame_out, "Forehead",
                       (x, y), cv2.FONT_HERSHEY_PLAIN, 1.5, col)
            #print("Done drawing. Returning %s" % self.face_rect)
            return
        if set(self.face_rect) == set([1, 1, 2, 2]):
            #print("Returning after Self face rect!! ")
            return
        cv2.putText(
            self.frame_out, "Press 'C' to change camera (current: %s)" % str(
                cam),
        ...
        forehead1 = self.get_subface_coord(0.5, 0.18, 0.25, 0.15)
        self.draw_rect(forehead1)
        #print("Forehead area identified")
